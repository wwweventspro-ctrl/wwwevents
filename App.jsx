...
export default function App() {
  ...
  async function processCheckinToken(token) {
    ...
    if (data.checked_in) {
      setCheckinResult({ ok: true, already: true, teamName: data.team_name });
      setView("checkin-result");
      return;
    }
    await supabase.from("teams").update({ checked_in: true }).eq("id", data.id);
    setCheckinResult({ ok: true, already: false, teamName: data.team_name });
    setView("checkin-result");
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4, flexWrap: "wrap" }}>
                  ...
                  {statuses[t.id] === "ouvert" && remainingSlots[t.id] > 0 && remainingSlots[t.id] <= 5 && (
                    <span style={styles.urgentBadge}>🔥 Plus que {remainingSlots[t.id]} place{remainingSlots[t.id] > 1 ? "s" : ""} !</span>
                  )}
                </div>
              </button>
            ))}
