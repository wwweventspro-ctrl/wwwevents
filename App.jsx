...
export default function App() {
  ...
  async function deleteTeam(tournamentId, teamId, teamName) {
    ...
    // Resynchro directe avec la base pour être sûr que la liste affichée est exacte
    const { data: freshTeams } = await supabase
      .from("teams")
      .select("*")
      .eq("tournament_id", tournamentId)
      .order("created_at", { ascending: true });
    setExpandedTeams((prev) => ({ ...prev, [tournamentId]: freshTeams || [] }));
    setTeamCounts((prev) => ({ ...prev, [tournamentId]: (freshTeams || []).length }));
